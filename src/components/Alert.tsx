...
export function Alert({ tone = 'info', title, children, icon, onDismiss, className }: AlertProps) {
  ...
  return (
    <div role={role} className={['ny-alert', `ny-alert--${tone}`, className].filter(Boolean).join(' ')}>
      {icon != null && (
      ...
      {onDismiss != null && (
        <button type="button" className="ny-alert__dismiss" aria-label="Dismiss" onClick={onDismiss}>
          <svg
            viewBox="0 0 24 24"
            width="14"
            height="14"
